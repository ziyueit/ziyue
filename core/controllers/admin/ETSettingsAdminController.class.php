...
public function action_index()
{
	...
	if ($form->validPostBack("save")) {
		...
		$config = array(
			...
			"esoTalk.registration.requireConfirmation" => in_array($v = $form->getValue("requireConfirmation"), array(false, "email", "approval","invite")) ? $v : false,
		);
...
protected function uploadHeaderImage($form)
{
     ...
     * 生成邀请码
     */
    public function action_create_invite(){
        $inviteModel = ET::inviteModel();
        $inviteModel->createCode(10);
        $this->redirect(URL("admin/settings/invite"));
    }
