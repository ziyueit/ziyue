...
public function action_general()
{
    ...
    $form->addField('signature','signature',array($this, "fieldSignature"), array($this, "savePersonal"));

    //增加个人博客
    $form->addSection("blog", T('Blog'));
    $form->addField('blog','blog',array($this, "fieldBlog"), array($this, "savePersonal"));

    //增加github
    $form->addSection("github", T('Github'));
    $form->addField('github','github',array($this, "fieldGithub"), array($this, "savePersonal"));

    //增加twitter
    $form->addSection("twitter", T('Twitter'));
    $form->addField('twitter','twitter',array($this, "fieldTwitter"), array($this, "savePersonal"));
    //增加Weibo
    $form->addSection("weibo", T('Weibo'));
    $form->addField('weibo','weibo',array($this, "fieldWeibo"), array($this, "savePersonal"));
    //增加Dribbble
    $form->addSection("dribbble", T('Dribbble'));
    $form->addField('dribbble','dribbble',array($this, "fieldDribbble"), array($this, "savePersonal"));
...
public function fieldSignature($form)
{
    ...
    $options = array("cols" => "10", "rows" => "5",'value'=>$signature['signature']);
	return $form->input("signature", "textarea", $options);
}
    public function fieldBlog($form)
    {
        $info = $this->profile("blog");
        return $form->input("blog",'text',array('value'=>$info['blog']));
    }
    public function fieldGithub($form)
    {
        $info = $this->profile("github");
        return $form->input("github",'text',array('value'=>$info['github']));
    }
    public function fieldTwitter($form)
    {
        $info = $this->profile("twitter");
        return $form->input("twitter",'text',array('value'=>$info['twitter']));
    }
    public function fieldDribbble($form)
    {
        $info = $this->profile("dribbble");
        return $form->input("dribbble",'text',array('value'=>$info['dribbble']));
    }
    public function fieldWeibo($form)
    {
        $info = $this->profile("weibo");
        return $form->input("weibo",'text',array('value'=>$info['weibo']));
    }
    public function fieldFacebook($form)
    {
        $info = $this->profile("facebook");
        return $form->input("facebook",'text',array('value'=>$info['facebook']));
    }
    /**
     * 设置个性签名
     * @param $form
     * @param $key
     */
public function savePersonal($form, $key)
    {
        $signature = $form->getValue($key);
        ET::memberModel()->updateById(ET::$session->userId, array($key => $signature));
    }
