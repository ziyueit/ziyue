...
public function action_join()
{
    ...
    // Add the invite code field to the form structure.
    if (C("esoTalk.registration.requireConfirmation") == 'invite'){
        $form->addSection("invite", T("Invite Code"));
        $form->addField("invite", "invite", function($form)
        {
        return $form->input("invite")."<br><small>".T("Need an invitation code")."</small>";
        },
        function($form, $key, &$data)
        {
            $data["invite"] = $form->getValue($key);
        });
    }
	...
	if ($form->validPostBack("submit")) {
		...
		if (!$form->errorCount()) {
            ...
            if (C("esoTalk.registration.requireConfirmation") == 'invite'){
                ...
                unset($data['invite']);
                $inviteModel = ET::inviteModel();
                if(!$inviteCode){
                    $this->renderMessage(T("Error!"), T("message.inviteCodeUnavailable"));
                    return;
                }
                $isUsed = $inviteModel->checkCode($inviteCode);
                if(!$isUsed){
                    $this->renderMessage(T("Error!"), T("message.inviteCodeUnavailable"));
                    return;
                }
			...
			if (!C("esoTalk.registration.requireConfirmation") or C("esoTalk.registration.requireConfirmation") == 'invite') $data["confirmed"] = true;
            ...
            //delete invite code
            if ($memberId && C("esoTalk.registration.requireConfirmation") == 'invite'){
                $inviteModel->deleteByCode($inviteCode);
